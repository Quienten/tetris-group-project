...
public final class TetrisBoard implements PropertyChangeEnabledBoard {
    ...
    public void newGame() {
        ...
        // Update the ghost piece on a new game.
        updateGhostPiece();
    ...
    public void down() {
        if (!move(myCurrentPiece.down())) {
            ...
            myPcs.firePropertyChange(PROPERTY_PIECE_LOCK, null, true);
            checkRows();
            if (!myGameOver) {
                myCurrentPiece = nextMovablePiece(false);

                // Update the ghost piece when myCurrentPiece changes after a piece lock.
                updateGhostPiece();
    ...
    /**
     * Update the ghost piece based off of the current piece.
     */
    private void updateGhostPiece() {

        //The ghost piece that resides under myCurrentPiece
        MovableTetrisPiece ghostPiece = myCurrentPiece;
        if (!myGameOver) {
            while (isPieceLegal(ghostPiece.down())) {
                ghostPiece = ghostPiece.down();
            }
            myPcs.firePropertyChange(PROPERTY_GHOST_PIECE, null, ghostPiece);
        }
    }
    ...
    private boolean move(final MovableTetrisPiece theMovedPiece) {
        ...
        if (isPieceLegal(theMovedPiece)) {
            ...
            // Update the ghost piece when myCurrentPiece moves or rotates.
            updateGhostPiece();
    ...
    private void checkRows() {
        ...
        if (!completeRows.isEmpty()) {
            ...
            if (linesCleared > 0) {
                myPcs.firePropertyChange(PROPERTY_LINES_CLEARED, null, linesCleared);
            }
            myPcs.firePropertyChange(PROPERTY_FROZEN_BLOCKS, null, myFrozenBlocks);
        }
    }
